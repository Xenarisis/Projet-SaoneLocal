<div
    x-data="{
        ...
        setStyle(t) {
            if(t === 'error') {
                this.color = 'bg-error dark:bg-red-400';
                this.icon = '{{ asset('images/notif/error.svg') }}';
            }
            else if(t === 'info') {
                this.color = 'bg-info dark:bg-blue-400';
                this.icon = '{{ asset('images/notif/info.svg') }}';
            }
            else if(t === 'warning' || t === 'warn') {
                this.color = 'bg-warning dark:bg-amber-400';
                this.icon = '{{ asset('images/notif/warn.svg') }}';
            }
            else {
                this.color = 'bg-success dark:bg-green-400';
    ...
    {{ $attributes->merge(['class' => 'fixed top-24 left-4 right-4 sm:left-auto sm:right-4 sm:w-full sm:max-w-sm z-50 flex items-start p-4 gap-3 bg-white border border-slate-200 rounded-lg shadow-lg dark:bg-slate-800 dark:border-slate-700']) }}
>
